<!-- Bootstrap modal -->
<div class="modal fade bs-example-modal-sm" id="modal_form" role="dialog">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h3 class="modal-title">Arsip Data Pasien</h3>
            </div>
            <div class="modal-body form">

                <?php
                    echo form_open('submit_ctrl',array('id'=>'arsiprm','class'=>'form-horizontal form-label-left'));
                ?>
                <div class="container cropper">
                    <div class="docs-data col-md-12 col-sm-12 col-xs-12">
                        <div class="input-group input-group-sm">
                            <?php
                            $skr = date("Y-m-d",now());
                            echo form_label('NO. RM*','drm',array('class'=>'input-group-addon'));
                            echo form_input(array('id' => 'drm', 'name' => 'drm','class'=>'form-control','readonly'=>'readonly'));
                            ?>
                        </div>
                        <?php
                            echo form_input(array('id' => 'dtgl', 'name' => 'dtgl','class'=>'form-control','value'=>$skr,'type'=>'hidden'));
                            echo form_input(array('id' => 'doprt', 'name' => 'doprt','class'=>'form-control','value'=>$operator['pgpid'],'type'=>'hidden'));
                        ?>

                        <div class="input-group input-group-sm">
                            <?php
                                echo form_label('RAK','drak',array('class'=>'input-group-addon'));
                                echo form_input(array('id' => 'drak', 'name' => 'drak','class'=>'form-control','placeholder'=>'[00]'));
                                echo '<span class="input-group-addon">BARIS</span>';
                                echo form_input(array('id' => 'dbar', 'name' => 'dbar','class'=>'form-control','placeholder'=>'[00]'));
                            ?>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-success" id="btnSave" onclick="sarsiprm();"><i class="glyphicon glyphicon-floppy-disk"></i> Simpan</button>
...
var table;
var nmnegara=<?php echo json_encode($this->dbcore1->get_negara()); ?>;
        var negArray = $.map(nmnegara, function(value, key) {
          return {
            value: value,
            data: key
          };
        });
        $('#autocomplete-negara').autocomplete({
          lookup: negArray,
          appendTo: '#autonegara'
        });
var nmpropinsi=<?php echo json_encode($this->dbcore1->get_propinsi()); ?>;
        var prpArray = $.map(nmpropinsi, function(value, key) {
          return {
            value: value,
            data: key
          };
        });
        $('#autocomplete-propinsi').autocomplete({
          lookup: prpArray,
          appendTo: '#autopropinsi'
        });
var nmkabupaten=<?php echo json_encode($this->dbcore1->get_kabupaten()); ?>;
        var kabArray = $.map(nmkabupaten, function(value, key) {
          return {
            value: value,
            data: key
          };
        });
        $('#autocomplete-kabupaten').autocomplete({
          lookup: kabArray,
          appendTo: '#autokabupaten'
        });
var nmkecamatan=<?php echo json_encode($this->dbcore1->get_kecamatan()); ?>;
        var kecArray = $.map(nmkecamatan, function(value, key) {
          return {
            value: value,
            data: key
          };
        ...
        $('#autocomplete-kecamatan').autocomplete({
          lookup: kecArray,
          appendTo: '#autokecamatan'
        });
var nmdesa=<?php echo json_encode($this->dbcore1->get_desa()); ?>;
        var desArray = $.map(nmdesa, function(value, key) {
          return {
            value: value,
            data: key
          };
        });
        $('#autocomplete-desa').autocomplete({
          lookup: desArray,
          appendTo: '#autodesa'
        });
...
$.blockUI();
  ...
  var url = "<?php echo base_url(); ?>markas/person/ajax_list";

  table = $('#tbpasien').DataTable({
            ...
            if( i < doc.content[1].table.body.length-1){
              if(i < 1){
                bod.push([
                  ...
                  {text:line[9].text, style:'tableHeader'},
                  {text:line[10].text, style:'tableHeader'},
                  {text:line[11].text, style:'tableHeader'}
                ]);
              } else {
                bod.push([
                  ...
                  {text:line[5].text, style:'defaultStyle1'},
                  {text:line[6].text, style:'defaultStyle1'},
                  {text:line[7].text, style:'defaultStyle1'},
                  {text:line[8].text, style:'defaultStyle1'},
                  {text:line[9].text, style:'defaultStyle1'},
                  {text:line[10].text, style:'defaultStyle1'},
                  {text:line[11].text, style:'defaultStyle1'}
                ]);
              }
            } else {
                bod.push([
                ...
                {text:line[9].text, style:'lastLine'},
                {text:line[10].text, style:'lastLine'},
                {text:line[11].text, style:'lastLine'}
              ]);
              }
            }
          );
...
function prosesarsip(id){

    $('#arsiprm')[0].reset(); // reset form on modals



    $.ajax({
        url : "<?php echo site_url('markas/person/editarsip/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
        if(data.pdpidpx){
            $('[name="drm"]').val(data.pdpidpx);
            $('[name="dtgl"]').val(data.pdppinjam);
            $('[name="doprt"]').val(data.pdpoprt);
            $('[name="drak"]').val(data.pdppos.substring(0,2));
            $('[name="dbar"]').val(data.pdppos.substring(2));
        } else {
            $('[name="drm"]').val(data.pxpidrs);
        }

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Penyimpanan Status'); // Set title to Bootstrap modal title

        },
        error: function (jqXHR, textStatus, errorThrown)
        {
            alert('Kesalahan AJAX, proses batal');
        }
    });
}
...
var url;
        ...
        url = "<?php echo site_url('markas/person/update_dtpx')?>";

    // ajax adding data to database
    $.ajax({
        url : url,
        type: "POST",
        data: $('#arsiprm').serialize(),
        dataType: "JSON",
        success: function(data)
...
function save() {
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('markas/person/ajax_add')?>";
    } else {
        url = "<?php echo site_url('markas/person/ajax_update')?>";
    }
...
function delete_person(id) {
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('markas/person/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
