...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$this->db->insert($this->tables['qmain_akses'], $data2);

		$data3 = array(
			'pgpid' => $identity,
			'pgpnama' => $additional_data['first_name'].' '.$additional_data['last_name'],
			'pgpalamat' => $additional_data['company'],
			'pgpemail' => $email,
			'pgkopar' => '01'.$additional_data['company']
		);
		$this->db->insert($this->tables['qmain_pgprofile_peg'], $data3);
